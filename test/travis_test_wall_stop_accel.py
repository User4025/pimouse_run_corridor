...
class WallStopAccelTest(unittest.TestCase):
    ...
    def test_io(self):
        ...
        left, right = self.set_and_get(30,19,0,0) #total: 49
        self.assertTrue(0< left == right <1000, "can't move again")

        time.sleep(5.0)
        left, right = self.set_and_get(30,19,0,0) #total: 49
        self.assertTrue(2000 < left == right, "can't accelrate")

        left, right = self.set_and_get(10,20,0,20) #total: 50
        self.assertTrue(left == right == 0,"can't stop")

if __name__ == '__main__':
    time.sleep(3)
    rospy.init_node('travis_test_wall_stop_accel')
    rostest.rosrun('pimouse_run_corridor','travis_test_wall_stop_aceel',WallStopAccelTest)
